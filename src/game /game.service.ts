...
export class GameService {
  ...
  private buildSuspicionScore(params: {
    ...
    if (
      ...
      suspicionScore += 3;
      reasons.push('high score in too short duration with robotic pattern');
    }
  ...
  private isValidClientSummary(params: {
    ...
    if (score < 0 || clicks < 0 || epicCount < 0 || melasCount < 0) {
      return false;
    }
  ...
  async finishGameV2(
    ...
    if (hasExtremeMismatch && !clientSummaryValid) {
      await this.invalidateGame({
        gameId,
        userId,
        reason:
